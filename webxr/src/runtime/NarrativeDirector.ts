...
export class NarrativeDirector {
  ...
  start(act: number) {
    ...
    this.timeline = new ActTimelinePlayer(this.emit, this.dispatcher, this.manifest)
    ...
    this.playPreBeatSubtitles(preCues)
  ...
  private playPreBeatSubtitles(preCues: { type: string; track_id?: string; at_sec: number }[]) {
    ...
    for (const cue of preCues) {
      ...
      const timeoutId = globalThis.setTimeout(() => {
        ...
        this.preLinePlayer = new VoiceoverLinePlayer(this.emit, trackId)
